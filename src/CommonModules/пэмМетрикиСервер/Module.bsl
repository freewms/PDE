...
Функция ПривестиТаблицуРасчетаМетрикиКСтрокеPrometheus(ИмяМетрики, ТипМетрики, МетрикаТаблицейЗначений) Экспорт
	...
	РазделительЦелойИДробнойЧасти = ",";	
	Если Константы.пэмРазделительЦелойИДробнойЧасти.Получить() = Перечисления.пэмРазделителиЦелойИДробнойЧасти.Точка Тогда
		РазделительЦелойИДробнойЧасти = ".";
	КонецЕсли;	
	...
	Попытка
		...
		Для Каждого Строка Из МетрикаТаблицейЗначений Цикл
			...
			ЗаписьJSON.ЗаписатьБезОбработки("} ");
			ЗаписьJSON.ЗаписатьБезОбработки(Строка(Формат(Строка["value"],"ЧРД="+РазделительЦелойИДробнойЧасти+"; ЧРГ=; ЧДЦ=4; ЧН=; ЧГ=;")));
			ЗаписьJSON.ЗаписатьБезОбработки(Символы.ПС);
