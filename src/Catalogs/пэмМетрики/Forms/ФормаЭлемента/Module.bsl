...
Процедура ВыполнитьПроверкуАлгоритмаРасчетаНаСервере()
	...
	сткВозврат = пэмМетрикиСервер.ПривестиТаблицуРасчетаМетрикиКСтрокеPrometheus(
		Объект.Код, 
		Объект.ТипМетрики, 
		сткВозврат.МетрикаТаблицей);
	
	Если сткВозврат.Ошибка Тогда
		
		Шаблон = "Процесс проверки расчета завершен с ошибкой на этапе преобразования.
		|Итоговая метрика: 
		|%1
		|Описание ошибки:
		|%2 ";
		
		Сообщить(СтрШаблон(Шаблон, сткВозврат.МетрикаСтрокой, сткВозврат.ОписаниеОшибки));
...
Процедура ОбновитьСостояниеФормы() Экспорт
	...
	Иначе
		Если Объект.Активность 
		И ПараметрыРегламентногоЗадания.Использование 
		И пэмМетрикиВызовСервера.РазрешеноИспользованиеМетодаPush() Тогда
