...
Функция ПолучитьМетрики(МетодПолученияМетрики) Экспорт
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ВЫБОР
	|				КОГДА РАЗНОСТЬДАТ(пэмСостояниеМетрик.ДатаРасчета, &ТекущаяДата, СЕКУНДА) >= РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), пэмМетрики.ПериодичностьРасчета, СЕКУНДА)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
...
//Таймаут - число. Квант раверн 0,1 секунды
//ВНИМАНИЕ! Компьютера с адресом 10.10.10.10 НЕ ДОЛЖНО БЫТЬ В ЛОКАЛЬНОЙ СЕТИ!!!
